...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      // The Make preview is served through an HTTPS proxy. Tell Vite's browser
      // client to reconnect through that same public origin instead of trying
      // an insecure local websocket endpoint.
      hmr: process.env.FIGMA ? {
        protocol: 'wss',
        clientPort: 443,
      } : undefined,
      watch: {
